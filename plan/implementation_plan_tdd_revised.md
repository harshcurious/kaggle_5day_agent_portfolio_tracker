...
2. **Masking ADK 2.0 Native Retries:**
   ...
   * *Correction:* Transient errors must be allowed to propagate out of the tool functions to allow the ADK framework to perform configured retries. Failsafe/partial status formatting must be handled at the *Node/Agent* level, not within the low-level tool adapters. Node-level failsafe wrappers must still re-raise framework control-flow interruptions such as cancellation or human-in-the-loop pauses.
3. **`ToolContext.state` Size Guidance:**
   * *Problem:* The original plan suggests having sub-agents write their full data payloads directly to `ToolContext.state`. ADK 2.0 guidelines warn that session state is intended for small serializable values and is not optimized for large data blobs.
   * *Correction:* The four data vector agents should pass their main payloads using node outputs, with `Event(output=...)` as the project convention. `ToolContext.state` should only store minimal session metadata such as the target `ticker`, request id, or retry/iteration counters. Do not store filings, transcripts, large vector outputs, or memo drafts in state.

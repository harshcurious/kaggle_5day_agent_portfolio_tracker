...
## 1. Assessment & Rating of Original Plan

### Overall Rating: **7/10 (Structurally Solid, Execution-Vulnerable)**
The original plan generated by gpt5.5 is highly detailed, well-structured, and defines clear business boundaries (the four data vectors, CIO synthesis, Critic loop, and TDD checks). However, it contains several **major engineering gaps** that would cause the implementation to hang, bypass native retry mechanisms, or violate ADK 2.0 state size limitations.

### Critical Pitfalls & Missed Details in the Original Plan

1. **The `JoinNode` Hang/Block Pitfall:**
   * *Problem:* ADK 2.0's `JoinNode` waits for *all* upstream parallel branches to yield an `Event`. If one of the branches (e.g., Sentiment Agent due to a missing EarningsCall API key) raises an unhandled exception or terminates without outputting an event, the `JoinNode` gets stuck, and the entire workflow hangs or crashes.
   * *Correction:* Every parallel node must be wrapped in a fallback handler at the node level to ensure it always yields/returns a failsafe `Event` containing a schema-compliant warning payload (e.g. `source_status = "failed"`), even if its underlying tool crashed.
2. **Masking ADK 2.0 Native Retries:**
   * *Problem:* The original plan suggests catching exceptions inside the low-level tool adapters to return failure status. In ADK 2.0, if tools catch all exceptions via broad `except Exception:` blocks, the framework's native `RetryConfig` (automatic retries) and Human-in-the-Loop (HITL) pauses are bypassed and disabled.
   * *Correction:* Transient errors must be allowed to propagate out of the tool functions to allow the ADK framework to perform configured retries. Failsafe/partial status formatting must be handled at the *Node/Agent* level, not within the low-level tool adapters.
3. **`ToolContext.state` Size Limits:**
   * *Problem:* The original plan suggests having sub-agents write their full data payloads directly to `ToolContext.state`. ADK 2.0 guidelines warn: *"The state parameter should not be used to persist large amounts of data between nodes. Use artifacts or other data persistence mechanisms."*
   * *Correction:* The four data vector agents must pass their main payloads using `Event(output=...)`. `ToolContext.state` should only store minimal session metadata (like the target `ticker` or current iteration attempt counts).
4. **Undefined `JoinNode` Output Format:**
   * *Problem:* The original plan does not define how the merged output of `JoinNode` is formatted and consumed by the CIO Agent, which can lead to brittle parsing code.
   * *Correction:* We introduce an explicit, deterministic `AggregatorNode` function after `JoinNode` to parse the collection of events, extract their payloads, and build a unified `SynthesizerInput` model for the CIO Agent.
5. **No Mock Testing Strategy:**
   * *Problem:* Testing yfinance, edgartools, and transcript APIs live will result in slow, flaky, and rate-limited tests, failing immediately if the developer lacks commercial API credentials.
   * *Correction:* Introduce a strict mock testing framework in Phase 0 using local JSON fixtures (`tests/fixtures/`) to ensure all test suites can run offline and in CI pipelines.
...
* [ ] Initialize project package with `uv init` and set up `pyproject.toml`.
...
* [ ] Define `InvestmentMemo`:
  ...
  * `supporting_evidence: dict` (mapped to vectors)
  * `key_risks: list[str]`
  * `data_gaps: list[str]`
...
* [ ] Implement `MacroNewsAdapter` wrapping Google Search Grounding.
...
* [ ] **Critical Failsafe Node Wrappers:** Wrap the Performance, Fundamental, Sentiment, and Macro agents in a node function that catches any final unhandled exceptions and yields a failsafe event:
  ```python
...
* [x] **No network operations during unit testing:** verified by socket patching in the test runner.
* [x] **No workflow hangs on API failures:** parallel branches are protected by failsafe node wrappers.
* [x] **No masked retries:** transient exceptions propagate to the framework, while permanent errors are caught at the node level.
* [x] **Compliance with state limits:** large payloads are passed as Event outputs, keeping `ToolContext.state` under 1KB of metadata.
* [x] **Deterministic Guardrails:** speculatory words and missing sections are checked using code rules before calling the LLM Critic.
* [x] **Mock execution mode:** CLI demo is runnable completely offline using local fixtures.
* [x] **Schema Integrity:** Pydantic schemas enforce type-safety and contract boundaries at every transition point.
